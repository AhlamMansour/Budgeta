...
public class BaseSeleniumTest {
	...
	protected void initSuite(@Optional("") String testPropertiesFileName) {
		...
		if (!testPropertiesFileName.equals("TestProps.properties")
				&& !testPropertiesFileName.equals("")) {
			SelTestLog
					.info("You provided the tests to read from proporties file: "
	...
	public void setEnv(@Optional("") String browser) {
		...
		myBrowser = browser;
		if (local) {
			baseURL = (baseURL == null) ? SelTestProps
					.get("common.base.url.local") : baseURL;
		} else {
			...
			if (testHost != null && testHost.length() > 10) {
				...
				baseURL = (baseURL == null) ? SelTestProps
						.get("common.base.url.remote") : baseURL;
			}
		}
		...
		driver = (WebDriver) TestParamsThreadPool
				.get(TestParamsThreadPool.KEY_WEB_DRIVER);
		if (driver == null) {
			driver = createNewDriver(null);
			TestParamsThreadPool.put(TestParamsThreadPool.KEY_WEB_DRIVER,
					driver);
		}
		...
		if (!local) {
			try {
				...
				Reporter.log("<div><hr/><b> Running " + className
						+ "on node id: " + nodeId + " Browser: "
						+ browserType.toString() + "</b>  <hr/></div>");
				SelTestLog.info("Running "
						+ TestParamsThreadPool.get(
								TestParamsThreadPool.TEST_NAME).toString()
						+ " on Node ID: " + nodeId);
	...
	protected void ignoreResultUponExpectedException(ITestResult result) {
		...
		testAnnotation = getAnnotationContainsText(result,
				"testng.annotations.Test");

		if (result.getMethod().getMethod().getDeclaredAnnotations().length >= 1) {
			knownIssueAnnotation = getAnnotationContainsText(result,
					"KnownIssue");
		}

		if (knownIssueAnnotation != null) {
			params = ((Method) (result.getMethod().getMethod()))
	...
	@SuppressWarnings("deprecation")
	private Annotation getAnnotationContainsText(ITestResult myResult,
			String text) {
		Annotation[] annotations = (Annotation[]) myResult.getMethod()
	...
	public void logMethodInner(ITestResult result, WebDriver driver)
			throws IOException {
		if ((result.getStatus() == ITestResult.FAILURE)
			...
			String PageSourceFileName = result.getMethod().getMethodName()
					+ new SimpleDateFormat("MM-dd-yyyy_HH-mm-ss")
							.format(new GregorianCalendar().getTime()) + ".txt";
			...
			try {
				...
				Reporter.log("<div>Test Class : "
						+ result.getMethod().getTestClass().getName()
						+ "</div>");
				Reporter.log("<div>Error on test: "
						+ result.getMethod().getMethodName() + " <br /></div>");
				Reporter.log("<img src=\"../" + failureImageFileName + ".png"
						+ "\" style=\"width:100%\" />");
				Reporter.log("<a href=\"../"
						+ (stored_report != null ? PageSourceFileName : "No")
						+ "\">page source</a>");
	...
	public static void takeScreenshot(WebDriver driver, String ImageFileName) {
		...
		try {
			File imageFile;
			if (local) {
				imageFile = ((TakesScreenshot) driver)
						.getScreenshotAs(OutputType.FILE);
			} else {
				WebDriver augmentedDriver = new Augmenter().augment(driver);
				imageFile = ((TakesScreenshot) augmentedDriver)
						.getScreenshotAs(OutputType.FILE);
			}
	...
	public WebDriver createNewDriver(UserAgent agent) {
		...
		if (chrome) {
			try {
				myDriver = SeleniumDriver.createDriver(BrowserType.CHROME,
						agent);
			} catch (WebDriverException wdEx) {
				wdEx.printStackTrace();
				System.err
						.println("tried creating chrome driver but WebDriverException was thrown, retrying... ");
				myDriver = SeleniumDriver.createDriver(BrowserType.CHROME,
						agent);
			}
		} else if (ie) {
	...
	public Object[][] getDataFromXmlFile(final Method testMethod) {
		DataProviderParams parameters = testMethod
				.getAnnotation(DataProviderParams.class);
