...
public class WebdriverUtils {
	...
	wait.until(new Function<WebDriver, Boolean>() {
	    ...
	    public Boolean apply(WebDriver arg0) {
		try {
		    return !arg0.findElement(By.className("busy")).isDisplayed();
		} catch (Exception e) {
		    return true;
		}
	    }
    ...
    public static String uploadExcelFile(WebElement uploadButton, WebDriver driver, String fileName, FileTypes fileType) {
	String fileNamex = "";
	if (SelTestProps.getBoolean("common.local")) {
	    File file;
	    file = new File(System.getProperty("user.dir") + "\\resources\\files\\" + fileName + "." + fileType.name().toLowerCase());
    ...
    public static void hoverOverElement(WebDriver driver, WebElement element, By elmLocator) {
	// Actions builder = new Actions(driver);
	/*
	 * if (driver instanceof ChromeDriver) { Actions hoverOverRegistrar = builder.moveToElement(element); hoverOverRegistrar.perform();
	...
	((JavascriptExecutor) driver).executeScript(js, element.findElement(elmLocator));
    ...
    /*
     * public boolean waitForPageLoad() throws InterruptedException { for (int second = 0;; second++) { if (second >= 60) return false; try { if
     * ("visibility: visible;".equals( selenium.getAttribute("top_right_form:connectStat:connection-idle@style" ))) { break; } } catch (Exception e) {
     * 
     * } Thread.sleep(1000); } }
     */

    public static String uploadFileFromResources(WebElement uploadButton, WebDriver driver, int pictureIndex) {
	return uploadFileFromResources(uploadButton, driver, pictureIndex, null);
    }

    private static String uploadFileFromResources(WebElement uploadButton, WebDriver driver, int pictureIndex, String path) {
	String fileName;
	if (SelTestProps.getBoolean("common.local")) {
	    File file;
	    if (path == null)
		file = new File(System.getProperty("user.dir") + "/resources/images/pic" + pictureIndex + ".jpg");
	    else
	    ...
	    String pathToFile = "";
	    if (path == null)
		pathToFile = "C://selenium//resources//images//pic" + pictureIndex + ".jpg";
	    else {
    ...
    /**
     * gets an element from a list using get(index) and waits until that element doesn't have a certain class.
     * 
     * @param driver
     * @param classToHave
	...
	wait.until(new Function<WebDriver, Boolean>() {
	    ...
	    public Boolean apply(WebDriver arg0) {
		try {
		    return !hasClass(classToHave, arg0.findElements(elementLocator).get(index));
		} catch (Exception e) {
		    return true;
		}
	    }
    ...
    public static void waitForElementInAListToNotHaveClassWithTime(WebDriver driver, final String classToHave, final By elementLocator, int waitTime) {
    ...
    public static void uploadSpecificFileFromResources(WebElement uploadButton, WebDriver driver, String relativePathWithFileName) {
	uploadFileFromResources(uploadButton, driver, -1, relativePathWithFileName);
    }
	...
	return new FluentWait<WebDriver>(driver).withTimeout(30, TimeUnit.SECONDS).pollingEvery(900, TimeUnit.MILLISECONDS);
    }
	...
	private final String floatCssVal;
     ...
     */
    public static Exception clickUntilAnotherElementIsFound(WebDriver driver, WebElement elementToClick, By by) {
    ...
    public static ExpectedCondition<WebElement> elementToHaveAttributeEqualTo(final WebElement elm, final String attribute, final String attributeString) {
	return new ExpectedCondition<WebElement>() {
	...
	wait.until(new Function<WebDriver, Boolean>() {
		...
		try {
		    ...
		    return (!disabledImageZone.getAttribute("style").contains("progress") && !disabledImageZone.getAttribute("style").contains("block") && !disabledImageZone
    ...
    public static void waitForForesightPOMLoad(WebDriver driver, final By objectLocator) {
	...
	wait.until(new Function<WebDriver, Boolean>() {
	    ...
	    public Boolean apply(WebDriver arg0) {
		try {
		    return hasAttributeEqualTo("display: none;", "style", arg0.findElement(locator));
		} catch (Exception e) {
		    return true;
		}
	    }
    ...
    /**
     * wait for the generated spinner to disappear while polling every 100 milliseconds.
     * 
     * @param driver
     */
	...
	try {
	    driver.findElement(By.className("j15")).findElement(By.className("generatedSpinner"));
	    return true;
	} catch (Exception e) {
	    return false;
	}
	...
	wait.until(new Function<WebDriver, Boolean>() {
		...
		try {
		    WebElement disabledImageZone = arg0.findElement(By.id("disabledImageZone"));
		    return (!disabledImageZone.getAttribute("style").contains("block") && !disabledImageZone.getAttribute("style").contains("cursor: wait"));
		} catch (Exception e) {
	...
	System.out.println(Float.parseFloat(elem.getCssValue("width").replaceAll("[^0-9.]", "")) + " / " + parentDivWIdth);
	return Math.round((Float.parseFloat(elem.getCssValue("width").replaceAll("[^0-9.]", "")) / parentDivWIdth) * 100);
    }
    ...
    public static boolean waitForTextToAppear(String text, int time, By textFieldLocator, WebDriver driver) {
	time = time * 100 - 500;
	sleep(500);
	while (time > 0) {
	    sleep(300);
	...
	return (ele.getCssValue("background-image").substring(ele.getCssValue("background-image").lastIndexOf("/") + 1, ele.getCssValue("background-image")
		.lastIndexOf(".")));
    }

    public static boolean WaitUntilClassDissappear(String className, WebElement element, int time) {
	time = time * 100 - 500;
	sleep(500);
	while (time > 0) {
	    sleep(300);
