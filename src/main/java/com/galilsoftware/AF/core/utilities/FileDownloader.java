...
public class FileDownloader {
	...
	private String downloader(WebElement element, String attribute, String downloadedFileName) throws IOException, NullPointerException, URISyntaxException {
        String fileToDownloadLocation = element.getAttribute(attribute);
        if (fileToDownloadLocation.trim().equals("")) throw new NullPointerException("The element you have specified does not link to anything!");
 
        URL fileToDownload = new URL(fileToDownloadLocation);
        File downloadedFile = new File(this.localDownloadPath + fileToDownload.getFile().replaceFirst("/|\\\\", "").replaceAll("\\|", "%7C"));
        if (downloadedFile.canWrite() == false) downloadedFile.setWritable(true);
 
        HttpClient client = new DefaultHttpClient();
        BasicHttpContext localContext = new BasicHttpContext();
 
        SelTestLog.info("Mimic WebDriver cookie state: " + this.mimicWebDriverCookieState);
        if (this.mimicWebDriverCookieState) {
            localContext.setAttribute(ClientContext.COOKIE_STORE, mimicCookieState(this.driver.manage().getCookies()));
        }
 
        HttpGet httpget = new HttpGet(fileToDownload.toString().replaceAll("\\|", "%7C"));
        HttpParams httpRequestParameters = httpget.getParams();
        httpRequestParameters.setParameter(ClientPNames.HANDLE_REDIRECTS, this.followRedirects);
        httpget.setParams(httpRequestParameters);
 
        SelTestLog.info("Sending GET request for: " + httpget.getURI());
        HttpResponse response = client.execute(httpget, localContext);
        this.httpStatusOfLastDownloadAttempt = response.getStatusLine().getStatusCode();
        SelTestLog.info("HTTP GET request status: " + this.httpStatusOfLastDownloadAttempt);
        SelTestLog.info("Downloading file: " + downloadedFile.getName());
        FileUtils.copyInputStreamToFile(response.getEntity().getContent(), new File(localDownloadPath + "\\" + downloadedFileName));
        response.getEntity().getContent().close();
 
        String downloadedFileAbsolutePath = downloadedFile.getAbsolutePath();
        SelTestLog.info("File downloaded to '" + downloadedFileAbsolutePath + "'");
    ...
    private String downloader(WebElement element, String attribute) throws IOException, NullPointerException, URISyntaxException {
        ...
        return downloader(element,attribute,"\\mydownloadedfile.xlsx");
    }
    
	public boolean DeleteFile(String downloadedFileName){
		try {
			File downloadedFile= new File(localDownloadPath + "\\" + downloadedFileName);
			if (downloadedFile.delete()) {
				System.out.println(downloadedFile.getName() + " is deleted!");
				return true;
			} else {
