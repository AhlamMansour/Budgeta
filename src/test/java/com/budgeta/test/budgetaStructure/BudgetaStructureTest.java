...
public class BudgetaStructureTest extends WrapperTest {
	...
	public void setBudgetTest() {
		...
		secondaryBoard = new SecondaryBoard();
		secondaryBoard.addSubLineForLine(OtherIncomeAndExpensesLine,
		...
		secondaryBoard.addSubLineForSubLine(OtherIncomeAndExpensesLine,
				OtherIncomeAndExpensesSubLine,
				OtherIncomeAndExpensesSub_SubLine);
		secondaryBoard = new SecondaryBoard();
		secondaryBoard.openAddChild(OtherIncomeAndExpensesSubLine, 2);
		subLine = new RevenuesAddSubLine();
		OtherIncomeAndExpensesSub_SubLine = WebdriverUtils
				.getTimeStamp(OtherIncomeAndExpensesSub_SubLine);
	...
	public void GeneralRevenuesTest(Hashtable<String, String> data) {
		...
		Assert.assertTrue(general.isDisplayed(),
				"expected general section to be displayed");
		...
		if (data.get("ShouldPass").equals("FALSE"))
			Assert.assertTrue(general.isGeneralHasError(),
					"expected to error in general section");
		else {
			...
			Assert.assertEquals(
					general.getDateRangeFrom(),
					BudgetaTest.getDateByNumbersFormat(
							data.get("DateRange_from_month"),
							data.get("DateRange_from_year")));
			Assert.assertEquals(
					general.getDateRangeTo(),
					BudgetaTest.getDateByNumbersFormat(
							data.get("DateRange_to_month"),
							data.get("DateRange_to_year")));
			Assert.assertEquals(general.getSelectedCurrency(),
					data.get("Currency"));
		}
	}
	...
	public void revenuesFormDetailsTest(Hashtable<String, String> data) {
		...
		if (data.get("ShouldPass").equals("FALSE")) {
			Assert.assertTrue(billings.isBillingsHasError(),
					"expected to error in billings section");
		} else {
			...
			if (data.get("SupportPercentage").isEmpty())
				...
				supportPercent = Integer
						.parseInt(data.get("SupportPercentage"));
			...
			monthX = BudgetaUtils.getMonthWithIndex(Integer.parseInt(general
					.getDateRangeFrom().split("/")[0]));
			yearX = general.getDateRangeFrom().split("/")[1];
			monthY = BudgetaUtils.getMonthWithIndex(Integer.parseInt(general
					.getDateRangeTo().split("/")[0]));
	...
	public void CostOfSaleTest(Hashtable<String, String> data) {
		...
		if (data.get("ShouldPass").equals("FALSE"))
			...
			Assert.assertEquals(general.getSelectedCurrency(),
					data.get("Currency"));
			Assert.assertEquals(general.getDepartment(),
	...
	public void CostOfSale_SalaryAndwagesTest(Hashtable<String, String> data) {
		board = new BudgetaBoard();
		secondaryBoard = board.getSecondaryBoard();
		secondaryBoard.clickOnSubLine(cost_of_revenues,
				cost_of_revenues_subLine, salary_and_wages);
		secondaryBoard = new SecondaryBoard();

		EmployeeAssumptions emplyeeAssumption = new EmployeeAssumptions();
		Assert.assertTrue(emplyeeAssumption.isDisplayed(),
				"expected employee assumption to be displayed");
		...
		emplyeeAssumption
				.setYearlyVacatoinDays(data.get("YearlyVactaionDayes"));
		emplyeeAssumption.setAvgAccruedVacation(data
				.get("AvgAccruedVacation_Percentage"));
		emplyeeAssumption.setMaxAccruedVacation(data
				.get("MaxAccruedVacation_Percentage"));
		emplyeeAssumption.setYearlyIncrease(data
				.get("YearlyIncrease_Percentage"));
		...
		if (data.get("ShouldPass").equals("FALSE"))
			Assert.assertTrue(emplyeeAssumption.employeeAssumptionHasError()
					|| general.isGeneralHasError(),
			...
			Assert.assertEquals(emplyeeAssumption.getGeography(),
					data.get("Geography"));
			Assert.assertEquals(emplyeeAssumption.getBenefits(),
					data.get("BenefitsPercentage"));
			Assert.assertEquals(emplyeeAssumption.getPayment(),
					data.get("Payment"));
			Assert.assertEquals(emplyeeAssumption.getYearlyVacatoinDays(),
					data.get("YearlyVactaionDayes"));
			Assert.assertEquals(emplyeeAssumption.getAvgAccruedVacation(),
					data.get("AvgAccruedVacation_Percentage"));
			Assert.assertEquals(emplyeeAssumption.getMaxAccruedVacation(),
					data.get("MaxAccruedVacation_Percentage"));
			Assert.assertEquals(emplyeeAssumption.getYearlyIncrease(),
					data.get("YearlyIncrease_Percentage"));
	...
	public void employeeFormTest(Hashtable<String, String> data) {
		...
		secondaryBoard = board.getSecondaryBoard();
		secondaryBoard.clickOnSubLine(cost_of_revenues,
				cost_of_revenues_subLine, salary_and_wages, employee);
		secondaryBoard = new SecondaryBoard();

		MainSection mainSection = new MainSection();
		Assert.assertTrue(mainSection.isDisplayed(),
				"expected to main section to be displayed");
		...
		emplyeeAssumption.setAvgAccruedVacation(data
				.get("AvgAccruedVactaion_Percentage"));
		emplyeeAssumption.setMaxAccruedVacation(data
				.get("MaxAccruedVactaionDays"));
		emplyeeAssumption.setYearlyIncrease(data
				.get("YeralyIncreasePercentage"));
		...
		if (data.get("ShouldPass").equals("FALSE"))
			Assert.assertTrue(
					mainSection.isMainSectionHasError()
							|| general.isGeneralHasError(),
					"expected to error in employee assumption sectcion or in general section");
		else {
			board.clickSaveChanges();
			...
			String monthFrom = BudgetaUtils.getMonthWithIndex(Integer
					.parseInt(dateFrom.split("/")[0]));
			String monthTo = BudgetaUtils.getMonthWithIndex(Integer
					.parseInt(dateTo.split("/")[0]));
			String payment = data.get("Payment");
			if (payment.isEmpty())
				payment = "Same month";
			general = new GeneralSection();
			String hireDate = general.getDateRangeFrom(), endDate = general
					.getDateRangeTo(), hireMonth, endMonth, hireYear, endYear;
			if (general.getDateRangeFrom().equals("MM/YYYY")) {
				hireDate = dateFrom;
				hireMonth = monthFrom;
				hireYear = yearFrom;
			} else {
				hireMonth = BudgetaUtils.getMonthWithIndex(Integer
						.parseInt(hireDate.split("/")[0]));
			...
			if (employeeSection.getSelecredTerm().equals("Monthly")) {
				String[] expectedValues = BudgetaUtils
						.calculateEmployeeValues_Monthly(monthFrom, yearFrom,
								monthTo, yearTo, hireMonth, hireYear, endMonth,
								endYear, baseSalary, benefits, bonus, payment);
				compareExpectedResults(expectedValues);
			}
			if (employeeSection.getSelecredTerm().equals("Yearly")) {
				String[] expectedValues = BudgetaUtils
						.calculateEmployeeValues_Yearly(monthFrom, yearFrom,
								monthTo, yearTo, hireMonth, hireYear, endMonth,
								endYear, baseSalary, benefits, bonus, payment);
				compareExpectedResults(expectedValues);
			}
	...
	public void OtherIncomeAndExpenses_OtherIncome(
		...
		if (data.get("ShouldPass").equals("FALSE")) {
			...
			monthX = BudgetaUtils.getMonthWithIndex(Integer.parseInt(dateFrom
					.split("/")[0]));
			monthY = BudgetaUtils.getMonthWithIndex(Integer.parseInt(dateTo
					.split("/")[0]));
